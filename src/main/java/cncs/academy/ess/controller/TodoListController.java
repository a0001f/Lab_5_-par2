...
public class TodoListController {
    ...
    public void getTodoList(Context ctx) {
        ...
        TodoList list = todoListService.getTodoList(listId, userId);
        if (list == null) {
    ...
    public void shareTodoList(Context ctx) {
        ...
        boolean success = todoListService.shareTodoList(listId, ownerId, request.targetUsername);
        if (success) {
            ctx.status(200);
        } else {
            ctx.status(400).json(new ErrorMessage("Could not share list"));
        }
    }

    public static class TodoListShareRequest {
